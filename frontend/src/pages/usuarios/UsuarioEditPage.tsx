...
import { getUsuario, updateUsuario, type UsuarioUpdateInput } from '@/services/usuarios.service';
...
export default function UsuarioEditPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [u, rs, ss] = await Promise.all([getUsuario(String(id)), listRoles(), listShippers()]);
        setRoles(rs);
        setShippers(ss);
  ...
  return (
    <DashboardLayout>
      <StandardPageLayout
        ...
        subtitle={userName ? `Modificando: ${userName}` : undefined}
        icon={<User className="h-5 w-5" />}
        actions={
          <div className="flex gap-2">
            <Button variant="ghost" size="sm" onClick={() => navigate(`/usuarios/${id}`)} className="gap-1.5">
              <ArrowLeft className="h-3.5 w-3.5" /> Volver
            </Button>
            <Button size="sm" type="submit" form="usuario-edit-form" disabled={submitting || loading} className="gap-1.5">
      ...
      >
        {loading ? (
          ...
          <form id="usuario-edit-form" onSubmit={submit} className="max-w-4xl mx-auto p-6 space-y-8">
            <SectionCard icon={Info} iconColor="blue" title="Información general">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2 md:col-span-2">
                  <Label className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Nueva contraseña (opcional)</Label>
                  <Input className="h-9" type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} />
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2 flex flex-col justify-end">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <Label htmlFor="activo" className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground cursor-pointer">
                      Usuario activo
                    </Label>
                  </div>
                </div>

...
import { createRol, listPermisos, type Permiso } from '@/services/roles.service';
...
export default function RolNewPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        setPermisos(await listPermisos());
      } catch (e) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await createRol({ nombre, permisos: selectedPermisos });
      navigate('/roles');
    } catch (e) {
  ...
  return (
    <DashboardLayout>
      ...
      >
        <form id="rol-new-form" onSubmit={submit} className="max-w-4xl mx-auto p-6 space-y-8">

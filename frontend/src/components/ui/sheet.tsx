...
const sheetVariants = cva(
    "fixed z-50 gap-4 bg-card p-6 sm:p-7 shadow-popover transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500 overflow-hidden",
    {
        variants: {
            side: {
...
>(({ side = "right", className, children, ...props }, ref) => {
    ...
    React.useEffect(() => {
        ...
        // Observer para detectar cuando el Sheet se abre/cierra
        const observer = new MutationObserver(() => {
            const isOpen = contentElement.getAttribute('data-state') === 'open';
            if (isOpen) {
                // Bloquear scroll en el body y en el layout principal
    ...
    return (
        <SheetPortal>
            ...
            <SheetPrimitive.Content
                ...
                onOpenAutoFocus={(e) => {
                    // Prevenir auto-focus que puede causar scroll
                    e.preventDefault();
                }}
                {...props}
            >
